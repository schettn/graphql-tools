...
describe('Execute: defer directive', () => {
  ...
  it('Does not execute deferred fragments early when not specified', async () => {
    ...
    expect(order).toEqual(['slow-id', 'fast-name']);
  });
  it('Does execute deferred fragments early when specified', async () => {
    ...
    expect(order).toEqual(['fast-name', 'slow-id']);
  });
  ...
  it('Initiates deferred grouped field sets only if they have been released as pending', async () => {
    ...
    expect(cResolverCalled).toBe(false);
    expect(eResolverCalled).toBe(false);
    ...
    expect(cResolverCalled).toBe(true);
    expect(eResolverCalled).toBe(false);
    ...
    expect(eResolverCalled).toBe(true);
  ...
  it('Initiates all deferred grouped field sets immediately once they have been released as pending', async () => {
    ...
    const { promise: cPromise, resolve: resolveC } = createDeferred<void>();
    let cResolverCalled = false;
    let eResolverCalled = false;
    const executeResult = execute({
      schema,
